<template>
  <metainfo>
    <template #title="{ content }">{{
      content ? `${content} - ${TITLE_SMALL_TEMPLATE}` : TITLE_TEMPLATE
    }}</template>
  </metainfo>
  ...
  <MessageContainer>
    <UpdateMessage
      v-if="update.updateExists.value"
      @update="update.refreshApp()"
    />
  </MessageContainer>
</template>
...
import {
  ...
  APP_ROUTE_NAMES,
  AUTH_LOGIN,
  LOGGED_USER_ALLOWED_ROUTE_NAMES,
...
import UpdateMessage from "@/src/components/messages/UpdateMessage";
...
export default {
  ...
  setup() {
    ...
    const themeColor = computed(() => {
      return exactTheme.value === THEME_LIGHT
        ? THEME_LIGHT_COLOR
        : THEME_DARK_COLOR;
    });
