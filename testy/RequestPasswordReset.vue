<template>
  <div class="auth-main__content">
    ...
    <template v-if="state === 'BEFORE' || state === 'SENDING'">
      <p class="subtitle">
        Wprowadź adres e-mail, aby zresetować hasło. Może być konieczne
...
import { emailSchema } from "@/src/configs/schemas";
import { BASE_URL } from "@/src/configs/url";

import { AUTH_REQUEST_PASSWORD_RESET } from "@/src/router/paths";
...
export default {
  ...
  setup(props) {
    useMeta({
      ...
      link: [
        { rel: "canonical", href: `${BASE_URL}${AUTH_REQUEST_PASSWORD_RESET}` },
      ],
    });
